...
int main(int argc, char** argv) {
  ...
  if (loadbin){
    graph.load_binary(prefix);
    graphlab::mpi_tools::finalize();
    return EXIT_SUCCESS;
  }

  if (loadjson) {
    graph.load_json(prefix, gzip, 
        graphlab::builtin_parsers::empty_parser<vertex_data>,
        graphlab::builtin_parsers::simple_parser<edge_data>);
    graphlab::mpi_tools::finalize();
    return EXIT_SUCCESS;
  }
